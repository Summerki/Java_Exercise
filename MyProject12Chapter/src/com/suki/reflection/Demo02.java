package com.suki.reflection;

import org.omg.PortableServer.ForwardRequest;

import java.lang.reflect.Constructor;
import java.lang.reflect.Field;
import java.lang.reflect.Method;

/**
 * 应用反射的API，获取类的信息(类的名字、属性、方法、构造器等)
 */
public class Demo02 {
    public static void main(String[] args) {
        String path = "com.suki.bean.User";
        try {
            Class clazz = Class.forName(path);

            // 获取类的名字
            System.out.println(clazz.getName()); // 获取包名+类名：com.suki.bean.User
            System.out.println(clazz.getSimpleName()); // 获取类名：User

            // 获取属性信息
//            Field[] fields = clazz.getFields(); // 只能获得public的field(属性)
            Field[] fields = clazz.getDeclaredFields(); // 获得所有的field
            Field f = clazz.getDeclaredField("uname");
            System.out.println(fields.length);
            for(Field temp : fields){
                System.out.println("属性:" + temp);
            }

            // 获取方法信息
//            Method[] methods = clazz.getMethods(); // 只能获取到public的方法
            Method[] methods = clazz.getDeclaredMethods(); // 获取所有的方法
            Method m01 = clazz.getDeclaredMethod("getUname", null);
            // 如果方法有参，则必须传递参数类型对应的class对象
            Method m02 = clazz.getDeclaredMethod("setUname", String.class);
            for(Method m : methods){
                System.out.println("方法:" + m);
            }

            // 获取构造器信息
            Constructor[] constructors = clazz.getDeclaredConstructors(); // 获取所有的构造器
            Constructor c = clazz.getDeclaredConstructor(int.class, int.class, String.class); // 获取指定构造器
            System.out.println("指定构造器:" + c);
            for(Constructor temp : constructors){
                System.out.println("构造器:" + temp);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
}
